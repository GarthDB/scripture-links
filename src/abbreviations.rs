...
pub fn create_abbreviation_map() -> AbbreviationMap {
    ...
    map.insert("DC", ("dc", StandardWork::DoctrineAndCovenants));
...
/// Canonical display name for a book slug (for wikilink targets).
/// Returns full book names for consistent Obsidian note matching.
#[must_use]
pub fn book_slug_to_display_name(slug: &str) -> Option<&'static str> {
    Some(match slug {
        // Old Testament
        "gen" => "Genesis",
        "ex" => "Exodus",
...
mod tests {
    ...
    #[test]
    fn test_book_slug_to_display_name_all_books() {
        let expected = vec![
            ("gen", "Genesis"),
            ("ex", "Exodus"),
            ("lev", "Leviticus"),
        ...
        for (slug, display) in expected {
            assert_eq!(
                book_slug_to_display_name(slug),
                Some(display),
                "slug '{slug}' should map to '{display}'"
            );
        }

        assert_eq!(book_slug_to_display_name("nonexistent"), None);
    }
